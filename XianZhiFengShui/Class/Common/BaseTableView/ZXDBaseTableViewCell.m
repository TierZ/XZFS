...
@implementation ZXDBaseTableViewCell{
    CGFloat sizeOnePx;
}
...
-(void)setCellType:(ZXDBaseCellType)type{
    _cellType = type;
    
    switch (_cellType) {
            
        case ZXDBaseCellNone:
            _topLineView.hidden = YES;
            _bottomLineView.hidden = YES;
            break;
            
        case ZXDBaseCellAtFirst:
        case ZXDBaseCellNormal:
        case ZXDBaseCellAtMiddle:
            _topLineView.hidden = NO;
            _bottomLineView.hidden = YES;
            break;
            
        case ZXDBaseCellAtLast:
        case ZXDBaseCellSingle:
            _topLineView.hidden = NO;
...
- (void)setCellData:(id)zxdCellData {
    _zxdCellData = zxdCellData;
}

- (void)setSeperatorLine:(NSIndexPath *)indexPath numberOfRowsInSection:(NSInteger)numberOfRowsInSection{
    if (numberOfRowsInSection == 1) {
        self.cellType = ZXDBaseCellSingle;
    }
    else {
        if (indexPath.row == 0) {
            self.cellType = ZXDBaseCellAtFirst;
        }
        else if (indexPath.row == numberOfRowsInSection - 1) {
            self.cellType = ZXDBaseCellAtLast;
        }
        else {
            self.cellType = ZXDBaseCellAtMiddle;
        }
    }
}
