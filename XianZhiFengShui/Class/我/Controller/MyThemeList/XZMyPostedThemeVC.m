...
@interface XZMyPostedThemeVC ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong)XZRefreshTable * myTheme;//发起的话题
@end
NSString * const myThemeCellId = @"myThemeCellId";
@implementation XZMyPostedThemeVC
...
-(void)setupTable{
    self.myTheme  =[[XZRefreshTable alloc]initWithFrame:CGRectZero style:UITableViewStylePlain];
    [self.view addSubview:self.myTheme];
    self.myTheme.sd_layout.spaceToSuperView(UIEdgeInsetsMake(0, 0, 0, 0));
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        [cell editThemeWithBlock:^(XZThemeListModel *model, NSIndexPath *indexPath) {
            NSLog(@"编辑block");
            [weakSelf.parentViewController.navigationController pushViewController:[[XZPostTopicVC alloc]init] animated:YES];
        }];
...
-(void)reloadTable{
    [self.myTheme.dataArray addObjectsFromArray:[self creatModelsWithCount:30]];
    [self.myTheme reloadData];
    [self.myTheme endRefreshHeader];
    [self.myTheme endRefreshFooter];
}
